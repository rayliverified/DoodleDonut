...
public class SharePlugin : MonoBehaviour
{
	...
	public void ShareImage (string subject, string subjectContent, string path)
	{
		#if UNITY_ANDROID
		if (Application.platform == RuntimePlatform.Android) {
			jo.CallStatic ("shareImage", subject, subjectContent, path);
		} else {
			AUP.Utils.Message (TAG, "warning: must run in actual android device");
		}
		#endif
	...
	public void SharePicture (string subject, string subjectContent)
	{
		#if UNITY_ANDROID
		if (Application.platform == RuntimePlatform.Android) {
			string screenShotName = "Doodle Donut.png";
			string folderPath = jo.CallStatic<String> ("createFolder", "Doodle Donut Screenshots", 0);
