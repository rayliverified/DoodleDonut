...
namespace SIS
{
	...
	public class IAPManager : MonoBehaviour, IStoreListener
	{
		...
		/// Optional: verify new product receipt online.
		/// </summary>
		public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs e)
		{
			...
			&& !string.IsNullOrEmpty(serverUrl))
            {
                MakeRequest(e.purchasedProduct);
        ...
        // that will forward the request to App Store servers
        private void MakeRequest(Product product)
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();
            #pragma warning disable 0219
                Dictionary<string, object> json = SIS.MiniJson.JsonDecode(product.receipt) as Dictionary<string, object>;
            ...
            #if UNITY_EDITOR
                PurchaseVerified(GetIAPIdentifier(product.definition.id));
                return;
            #elif UNITY_ANDROID
                json = SIS.MiniJson.JsonDecode(json["Payload"] as string) as Dictionary<string, object>;
                json = SIS.MiniJson.JsonDecode(json["json"] as string) as Dictionary<string, object>;
                dic.Add("store", "Android");
                dic.Add("pid", json["productId"] as string);
                dic.Add("tid", json["orderId"] as string);
                dic.Add("rec", json["purchaseToken"] as string);

                IAPObject obj = GetIAPObject(dic["pid"]);
                if (obj != null && obj.type == ProductType.Subscription)
                    dic.Add("type", "subs");    
            #elif UNITY_IOS
                dic.Add("store", "IOS");
			    dic.Add("pid", product.definition.storeSpecificId);
                dic.Add("rec", json["Payload"] as string);
            #endif
        ...
        IEnumerator WaitForRequest(Dictionary<string, string> dic)
        {
            ...
            if (www.error == null)
            {
				Product p = controller.products.WithStoreSpecificID(id);
				if (p != null)	controller.ConfirmPendingPurchase (p);

                //we have a successful response from our server,
                //but it returned false (fake purchase)
                if (!bool.Parse(www.text))
                {
                    if (isDebug) Debug.Log("The receipt for '" + id + "' could not be verified: " + www.text);
                ...
                else
				{
                    ...
                    if (isDebug) Debug.Log(dic["pid"] + " verification success.");
                }
            }
            else
            {
                ...
                if (isDebug) Debug.Log("Verification URL error: " + www.error + ", " + www.text);
            }
